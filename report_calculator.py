...
class ReportCalculator:
    ...
    def get_attr_values(self, weather_dataset, attr, year, month=-1):
    ...
    def get_value(self, weather_dataset, attr, value_type, year, month=-1):
        """calculates the provided value on given year and month
        """

        attr_values = self.get_attr_values(weather_dataset, attr, year, month)
        limit_value_function = min if value_type == "min" else max

        return limit_value_function(attr_values, key=itemgetter(0))

    def get_mean_attr(self, weather_dataset, attr, year, month=-1):
